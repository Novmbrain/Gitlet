 ...
 * @className: Handler4
 * @description:
 * Handle the case where the file is unmodified in Other but not present in HEAD → Remain removal, do nothing
 * @author: Wenjie FU
 * @date: 15/02/2024
 **/
public class MergeHandler4 implements MergeHandler {
    @Override
    public boolean handle(String fileName,
        ...
        if (!headCommit.containsFile(fileName)
            ...
            if (givenCommit.isFileEqual(fileName, splitFileHash)) {
                // Do nothing
                handled = true;
            }
        }
